...
| 미션                    | 배점 | 감점 기준 (해당 시 누적 감점)                                                                                                                                                     |
| --------------------- | -: | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **1. 데이터 신뢰도 점검**     |  7 | • 결측치·이상치 점검 자체 없음 (-2)<br>• 결측/이상치 처리 **기준** 불명확 (-2)<br>• 처리 전·후 비교(표/요약) 없음 (-1)<br>• 변수 범위 현실성(age, bmi 등) 점검 누락 (-1)<br>• charges 단위(USD)·현실성 평가 없음 (-1)          |
| **2. 분포 첫인상 해석**      |  6 | • 분포 시각화(히스토그램/KDE) 없음 (-2)<br>• 그래프 제시 후 해석 문장 없음 (-2)<br>• 왜도·long-tail 언급 없음 (-1)<br>• 변환 필요성 판단/언급 없음 (-1)                                                         |
| **3. 로그·비선형 변환 비교**   |  6 | • 변환 전·후 비교 시각화 없음 (-3)<br>• 변환 선택 이유 미설명 (-2)<br>• 변환이 해석에 주는 영향 언급 없음 (-1)                                                                                           |
| **4. 성별·지역별 분석**      |  7 | • 범주별 시각화 없이 수치만 제시 (-2)<br>• 분포 겹침/차이에 대한 해석 없음 (-2)<br>• 중앙값·산포 비교 언급 없음 (-1)<br>• 사회·제도적 맥락 추정 없음 (-1)<br>• 결과를 단정적으로 일반화 (-1)                                      |
| **5. 흡연 효과 검정**       |  7 | • 가설검정 수행 안 함 (-2)<br>• 검정 방법 선택 이유 미설명 (-1)<br>• 효과 크기(차이의 크기) 제시 없음 (-2)<br>• 시각화 없이 검정 결과만 제시 (-1)<br>• 정책·의료적 의미 해석 없음 (-1)                                        |
| **6. 연령대별 비교(ANOVA)** |  7 | • 연령 구간 설정 근거 없음 (-1)<br>• ANOVA만 수행, 사후검정 없음 (-2)<br>• 사후검정 결과 해석 없음 (-1)<br>• 연령대 간 차이 구조적 정리 없음 (-2)<br>• 분포 시각화 없음 (-1)                                            |
| **7. 상관·다중공선성**       |  7 | • 상관계수 계산 없음 (-2)<br>• VIF 계산 또는 논의 없음 (-2)<br>• 공선성 문제 여부 판단 없음 (-1)<br>• 공선성 대응 전략 비교·언급 없음 (-2)                                                                     |
| **8. 연속형 변수 상관 해석**   |  7 | • Pearson/Spearman 중 하나만 제시 (-2)<br>• 계수 선택 이유 미설명 (-2)<br>• 의료·보건 맥락 해석 없음 (-1)<br>• 상관을 인과로 오해하는 서술 (-2)                                                             |
| **9. 상관 vs 인과 구분**    |  6 | • 상관·인과 차이 언급 없음 (-2)<br>• 대리변수/누락변수 개념 미언급 (-2)<br>• 데이터 한계(미측정 요인) 언급 없음 (-1)<br>• 정책·해석상 주의점 없음 (-1)                                                                |
| **10. 회귀모형 설계**       | 11 | • 변수 선택 이유 미기재 (-3)<br>• 범주형 변수 처리 기준 불명확 (-2)<br>• 변환 변수 해석 고려 없음 (-2)<br>• R² 등 기본 지표 해석 없음 (-2)<br>• 설계가 앞선 분석 결과와 연결되지 않음 (-2)                                     |
| **11. 회귀계수 해석**       | 11 | • 계수 해석 문장 자체 없음 (-3)<br>• 계수를 수치 나열로만 제시 (-2)<br>• 의료비 의미로 번역하지 않음 (-2)<br>• 범주형 변수 기준 범주 미명시 (-2)<br>• 변환 변수 해석 오류 (-2)                                              |
| **12. 모형 진단·개선**      |  8 | • 잔차 진단 그래프(QQ·잔차플롯 등) 없음 (-2)<br>• 가정 위반 여부 판단 없음 (-1)<br>• 캘리브레이션(예측 vs 실제) 언급 없음 (-1)<br>• 오차 크기의 현실적 의미 해석 없음 (-1)<br>• 개선 아이디어 제시 없음 (-1)<br>• 극단치/영향력 논의 없음 (-2) |
| **13. 스토리텔링 종합**      | 10 | • 가상 개인 사례 제시 없음 (-3)<br>• 의료비 차이를 계수로 설명 못함 (-3)<br>• 모델이 설명 못하는 요소 언급 없음 (-2)<br>• 비전공자에게 이해 어려운 서술 (-1)<br>• 분석 흐름 단절 (-1)                                          |
