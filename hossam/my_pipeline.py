...
def _qtcheck_result_tabs(df, dup_count, dup_removed):
    ...
    # 기술통계 (명목형 / 연속형 드롭다운 페이지)
    secs = []
    if category_cols:
        secs.append(("명목형 기술통계량",
                     lambda: display(my_qtcheck.categorical_summary(
                         df, columns=category_cols, value_counts=False))))
    if number_cols:
        secs.append(("연속형 기술통계량",
                     lambda: display(my_qtcheck.numerical_summary(df, columns=number_cols).T)))
    if secs:
...
def _qtcheck_noninteractive(data):
    ...
    if cats:
        display(my_qtcheck.categorical_summary(df, columns=cats, value_counts=False))
    if nums:
        display(my_qtcheck.numerical_summary(df, columns=nums).T)

    _qtcheck_insight(df, dup_count, dup_removed=(dup_count > 0))


def qtcheck_pipeline(data):
    ...
    step2_out = _Output()
    append(o2, _VBox(checks), convert_btn, step2_out)
